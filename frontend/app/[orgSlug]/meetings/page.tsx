...
export default function MeetingsPage() {
  ...
  const enrichMeetings = useCallback(async (items: Meeting[]) => {
    const enriched = await Promise.all(
      items.map(async (m) => {
        ...
        try {
          const detail = await apiClient.get<{ decisions: unknown[] }>(`/api/meetings/${m.id}`);
          return { ...m, decisionsCount: detail.decisions?.length ?? 0 };
        } catch {
          return { ...m, decisionsCount: 0 };
        }
      }),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("meetings")}
        description="Enregistrez et analysez vos réunions avec l'IA."
      />

      <form onSubmit={handleUpload} className="tb-card p-6">
        <div>
          <label className="tb-label" htmlFor="meeting-title">
            Titre de la réunion
          </label>
          <input
            id="meeting-title"
        ...
        >
          ...
          <p className="mt-1 text-xs text-slate-500">ou cliquez pour sélectionner</p>
          {audioFile && (
        ...
        <button
          ...
          className="tb-btn-primary mt-4 h-10"
        >
      ...
      {meetings.length === 0 ? (
        <EmptyState
          ...
          description="Téléversez un enregistrement audio pour extraire décisions et tâches automatiquement."
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {meetings.map((m) => {
            ...
            return (
              <article key={m.id} className="tb-card p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={cn("shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium", st.className)}>
                    {st.label}
                  </span>
                </div>
                <div className="mt-4 flex items-center justify-between">
                  ...
                  {m.decisionsCount > 0 && (
                    <span className="text-xs text-slate-500">
                      {m.decisionsCount} décision{m.decisionsCount > 1 ? "s" : ""}
                    </span>
                  )}
                </div>
                {m.ai_summary && (
                  <p className="mt-3 line-clamp-3 text-sm text-slate-600 dark:text-slate-400">{m.ai_summary}</p>
                )}
              </article>
            );
          })}
